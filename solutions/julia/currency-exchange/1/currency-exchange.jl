...
function get_value_of_bills(denomination, number_of_bills)
    round(denomination * number_of_bills)
end

function get_number_of_bills(amount, denomination)
    ceil(amount ÷ denomination)
end
...
function exchangeable_value(budget, exchange_rate, spread, denomination)
    exchangeValue = exchange_money(budget, exchange_rate * (1 + spread / 100))
    get_value_of_bills(denomination, get_number_of_bills(exchangeValue, denomination))
end
